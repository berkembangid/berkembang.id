...
function PageHeader({ title, description }: { title: string; description: string }) {
  return <div><h1 className="text-xl font-black text-slate-800 md:text-2xl">{title}</h1><p className="mt-1 text-xs text-slate-500 md:text-sm">{description}</p></div>;
}
function Disclaimer({ children }: { children: string }) {
  return <div className="flex gap-3 rounded-2xl border border-blue-200 bg-blue-50 p-4 text-xs leading-relaxed text-blue-900"><ShieldCheck className="shrink-0" size={18} /><p>{children}</p></div>;
}
function ScoreView({ data }: { data: ReadinessView }) {
  return <main className="mx-auto max-w-5xl space-y-6 p-4 pb-28 md:p-6 md:pb-8"><PageHeader title="Kesiapan Data Usaha" description="Lihat data yang sudah tercatat dan yang masih perlu dilengkapi." />
    <section className="flex flex-col items-center gap-6 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:flex-row"><div className="flex h-28 w-28 shrink-0 flex-col items-center justify-center rounded-2xl bg-[#0f2d6b] text-white"><strong className="text-4xl">{Math.round(data.score)}</strong><span className="text-[10px] text-blue-100">dari 100</span></div><div className="flex-1"><h2 className="font-bold text-slate-800">Nilai berdasarkan bukti yang tersedia</h2><p className="mt-2 text-xs leading-relaxed text-slate-500">Nilai kosong tidak dianggap gagal. Sistem menunggu data yang cukup sebelum memberi nilai.</p></div><Link href="/umkm/gaps" className="inline-flex items-center gap-2 rounded-xl bg-[#0f2d6b] px-4 py-3 text-xs font-bold text-white">Lihat yang perlu dilengkapi <ArrowRight size={14} /></Link></section>
...
function GapsView({ data }: { data: ReadinessView }) {
  ...
  return <main className="mx-auto max-w-5xl space-y-6 p-4 pb-28 md:p-6 md:pb-8"><PageHeader title="Yang Perlu Dilengkapi" description="Mulai dari satu langkah yang paling membantu. Tidak perlu dikerjakan sekaligus." />
    ...
    <Link href="/umkm/roadmap" className="inline-flex items-center gap-2 text-xs font-bold text-blue-800">Lihat urutan langkah usaha <ArrowRight size={14} /></Link><Disclaimer>{data.disclaimer}</Disclaimer></main>;
}
function RoadmapView({ data }: { data: ReadinessView }) {
  const completed = data.missions.filter((item) => item.status === "completed").length;
  return <main className="mx-auto max-w-5xl space-y-6 p-4 pb-28 md:p-6 md:pb-8"><PageHeader title="Langkah Usaha Saya" description="Tugas selesai otomatis ketika buktinya sudah tercatat." />
    {data.primaryMission && <section className="rounded-2xl bg-[#0f2d6b] p-6 text-white"><p className="text-[10px] font-bold uppercase tracking-widest text-blue-200">Langkah yang disarankan sekarang</p><h2 className="mt-2 text-lg font-bold">{data.primaryMission.title}</h2><p className="mt-1 text-xs leading-relaxed text-blue-100">{data.primaryMission.description}</p><Link href={data.primaryMission.href} className="mt-4 inline-flex items-center gap-2 rounded-xl bg-white px-4 py-2.5 text-xs font-bold text-blue-900">Mulai langkah ini <ArrowRight size={14} /></Link></section>}
