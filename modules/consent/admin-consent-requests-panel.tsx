...
export default function AdminConsentRequestsPanel() {
  ...
  /**
   * Hanya kegagalan MEMUAT daftar yang masih tinggal di layar. Sebelumnya
   * kotak biru yang sama dipakai untuk keberhasilan dan kegagalan sekaligus,
   * jadi "Akses belum dapat dicabut" tampil dengan warna dan penanda yang
   * persis sama dengan "Akses berhasil dicabut".
   */
  ...
  async function decide(decision: "approve" | "reject") {
    if (!selected) return;
    // Menyetujui membuka identitas dan kontak sebuah UMKM kepada lembaga, dan
    // snapshot profilnya dibuat saat itu juga. Keduanya tidak bisa ditarik
    // kembali dengan menekan tombol yang lain.
    const yes = await confirm({
      title: decision === "approve" ? "Setujui permintaan akses ini?" : "Tolak permintaan akses ini?",
      description: decision === "approve"
        ? `${selected.institutionName} akan melihat identitas dan kontak UMKM sesuai ruang lingkup yang diminta, dan snapshot profilnya dibuat sekarang juga.`
        : `${selected.institutionName} tidak akan mendapatkan akses. Permintaan yang sama masih bisa diajukan lagi nanti.`,
    ...
    try {
      ...
      setSelected(null);
      notifySuccess(decision === "approve" ? "Akses disetujui" : "Permintaan ditolak", {
        description: decision === "approve" ? "Snapshot profil sudah dibuat." : undefined,
      });
      await load();
    } catch (error) {
      notifyFromError(error, "Keputusan belum dapat disimpan.");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function revoke(grantId: string) {
    const yes = await confirm({
      title: "Cabut akses ini sekarang?",
      description: "Lembaga langsung kehilangan akses ke profil UMKM tersebut. Snapshot yang sudah pernah diunduh tidak bisa ditarik kembali, dan pencabutan ini tercatat di audit.",
    ...
    try {
      ...
      notifySuccess("Akses dicabut");
      await load();
    } catch (error) { notifyFromError(error, "Akses belum dapat dicabut."); }
