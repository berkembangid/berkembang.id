...
export default function InstitutionProfilesPage() {
  ...
  return <DashboardPage>
    ...
    {openedId && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/50 p-4" onMouseDown={(event) => { if (event.target === event.currentTarget) { setOpenedId(null); setDetail(null); setDetailError(null); } }}>
      <section role="dialog" aria-modal="true" className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
        {detailLoading && (
          <div className="py-16 text-center">
            <div className="inline-block h-8 w-8 animate-spin rounded-full border-4 border-solid border-[#0b5f86] border-r-transparent align-[-0.125em]" />
            <p className="mt-3 text-sm text-slate-600">Membuka snapshot beku dossier...</p>
          </div>
        )}
        {detailError && !detailLoading && (
          <div className="py-8 text-center">
            ...
            <h3 className="mt-3 text-base font-bold text-slate-900">Dossier Tidak Dapat Dibuka</h3>
            <p className="mt-1 text-sm text-slate-600 max-w-md mx-auto">{detailError}</p>
            <button
              onClick={() => { setOpenedId(null); setDetail(null); setDetailError(null); }}
              className="mt-6 rounded-xl bg-slate-900 px-6 py-2.5 text-xs font-bold text-white hover:bg-slate-800"
            >
              Tutup
            </button>
          </div>
        ...
        {detail && <>
          <div className="flex items-start justify-between gap-4">
            ...
            <span className={`rounded-full px-2.5 py-1 text-xs font-bold ${detail.header.downloadAllowed ? "bg-emerald-50 text-emerald-700" : "bg-amber-50 text-amber-700"}`}>
              {detail.header.downloadAllowed ? "Unduhan diizinkan" : "Hanya lihat"}
            </span>
          </div>

          <Block title="Tingkat kesiapan">
            <p className="text-sm font-bold text-slate-800">Tingkat: {String(detail.readiness.state?.level ?? (detail.readiness.snapshot as Record<string, unknown> | undefined)?.score ?? (detail.readiness.snapshot as Record<string, unknown> | undefined)?.level ?? "—")}</p>
            <p className="mt-1 text-xs text-slate-500">Sejak {date(detail.readiness.state?.level_since ?? null)} · dihitung {date(((detail.readiness.snapshot as Record<string, unknown> | undefined)?.calculatedAt as string) ?? null)}</p>
          </Block>

          <Block title="Keuangan 6 bulan (ringkas)">
            <div className="grid grid-cols-2 gap-2 text-xs">
              <Info label="Pemasukan" value={`Rp${num((detail.financial6m.summary as Record<string, unknown> | undefined)?.incomeTotal)}`} />
              <Info label="Pengeluaran" value={`Rp${num((detail.financial6m.summary as Record<string, unknown> | undefined)?.expenseTotal)}`} />
              <Info label="Transaksi" value={num((detail.financial6m.summary as Record<string, unknown> | undefined)?.transactionCount)} />
              <Info label="Hari aktif" value={num((detail.financial6m.summary as Record<string, unknown> | undefined)?.activeDays)} />
            </div>
          ...
          <Block title="Jejak dokumen (report_issues)">
            {(!detail.reportTrail || detail.reportTrail.length === 0) && <p className="text-xs text-slate-500">Belum ada PDF yang diterbitkan ke lembaga ini.</p>}
          ...
          <button onClick={() => { setOpenedId(null); setDetail(null); setDetailError(null); }} className="mt-5 min-h-11 w-full rounded-xl bg-[#0b5f86] text-sm font-bold text-white hover:bg-[#094c6b]">Tutup</button>
        </>}
      </section>
    </div>}
