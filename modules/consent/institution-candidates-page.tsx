...
export default function InstitutionCandidatesPage() {
  ...
  return <main className="p-5 md:p-8">
    <div className="mb-6 flex flex-wrap items-start justify-between gap-3"><div><h1 className="font-headline text-2xl font-bold text-[#141a34]">Cari usaha yang sesuai</h1><p className="mt-1 max-w-2xl text-sm text-slate-600">Informasi di bawah masih anonim. Identitas dan ringkasan usaha baru terlihat setelah pemilik memberi izin.</p></div><span className="inline-flex items-center gap-2 rounded-full bg-emerald-50 px-3 py-2 text-xs font-bold text-emerald-700"><ShieldCheck size={15}/>Privasi terjaga</span></div>
    {message && <div role="status" className="mb-5 rounded-xl border border-blue-200 bg-blue-50 p-3 text-sm text-blue-900">{message}</div>}
    <div className="mb-5 flex flex-wrap gap-2" aria-label="Saring berdasarkan bidang usaha">{sectors.map((item) => <button key={item} onClick={() => setSector(item)} className={`min-h-10 rounded-full px-4 text-xs font-bold ${sector === item ? "bg-[#001b85] text-white" : "border border-slate-300 bg-white text-slate-600"}`}>{item}</button>)}</div>
    {loading ? <p className="py-16 text-center text-sm text-slate-500">Memuat daftar usaha...</p> : <div className="grid gap-4 lg:grid-cols-2">{visible.map((candidate) => <article key={candidate.businessId} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-3"><div className="flex items-center gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-xl bg-blue-50 text-[#001b85]"><Building2 size={21}/></span><div><h2 className="font-bold text-slate-900">{candidate.candidateCode}</h2><p className="text-xs text-slate-500">{candidate.sector} · Usaha {candidate.businessAge}</p></div></div><span className="rounded-full bg-emerald-50 px-2.5 py-1 text-xs font-bold text-emerald-700">{candidate.readinessBand}</span></div>
      ...
      <button disabled={candidate.requestStatus === "pending" || candidate.dossierStatus === "ready"} onClick={() => { setSelected(candidate); setScopes(defaultScopes); }} className="mt-5 min-h-11 w-full rounded-xl bg-[#001b85] px-4 text-sm font-bold text-white disabled:bg-slate-200 disabled:text-slate-500">{candidate.dossierStatus === "ready" ? "Izin masih aktif" : candidate.requestStatus === "pending" ? "Menunggu jawaban pemilik" : "Minta izin melihat profil"}</button>
    </article>)}</div>}
