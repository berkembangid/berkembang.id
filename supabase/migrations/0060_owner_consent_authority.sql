-- ---------------------------------------------------------------------------
-- 0060 — Pemilik usaha memutuskan sendiri siapa yang boleh melihat datanya
-- ---------------------------------------------------------------------------
-- `0048` memindahkan persetujuan dan pencabutan akses dossier dari pemilik
-- usaha ke admin platform: kedua fungsinya dibuka dengan
-- `if not private.is_platform_admin() then raise ...`.
--
-- Itu menghapus satu-satunya kendali yang dijanjikan produk ini kepada
-- pemiliknya. Layar masuk menuliskannya sebagai janji -- "Data tetap dalam
-- kendali Anda" -- dan seluruh modul persetujuan dibangun di atas gagasan
-- bahwa yang memutuskan adalah orang yang datanya dibicarakan. Persetujuan
-- yang hanya bisa diberikan pihak lain bukan persetujuan.
--
-- Akibat teknisnya juga sudah terlihat: `db:test` berhenti di
-- REQUEST_NOT_FOUND karena skenarionya menjalankan persetujuan sebagai
-- pemilik, sebagaimana seharusnya.
--
-- Yang dipulihkan di sini BUKAN pembalikan pekerjaan admin. Keduanya
-- berlaku: pemilik boleh memutuskan tentang datanya sendiri, dan admin
-- platform tetap boleh mendampingi. Layar admin yang dibuat `0048` terus
-- bekerja apa adanya.
--
-- Pencabutan adalah bagian yang paling tidak boleh ditawar. Sebuah usaha
-- yang ingin menghentikan akses lembaga terhadap pembukuannya tidak boleh
-- menunggu siapa pun.
...
begin
  ...
  if private.business_role(grant_row.business_id) <> 'owner'
    and not private.is_platform_admin() then
