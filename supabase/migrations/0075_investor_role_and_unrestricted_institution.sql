-- Migration: 0075_investor_role_and_unrestricted_institution.sql
-- Description: Mendukung peran Investor/Offtaker dan pemisahan akses data UMKM antara Lembaga dan Investor

-- 1. Tambahkan parameter p_search pada list_anonymous_business_candidates
-- Serta logika: jika institusi bertipe bukan investor (Lembaga/Bank/Instansi resmi), tampilkan nama usaha asli dan tidak disensor.
...
declare
  ...
  is_investor_bool boolean := false;
  result_value jsonb;
  total_value integer;
  clean_search text;
begin
  ...
  -- Periksa jenis institusi
  select coalesce(type, '') into institution_type_value
  from public.institutions
  where id = institution_id_value;

  if lower(institution_type_value) like '%investor%' or lower(institution_type_value) like '%offtaker%' then
    is_investor_bool := true;
  end if;
  ...
  with candidates as (
    select
      case 
        when is_investor_bool then optin.candidate_code
        else coalesce(business.name, optin.candidate_code)
      end as "candidateCode",
      -- Sertakan field identitas asli untuk lembaga resmi
      case 
        when is_investor_bool then null
        else business.name
      end as "businessName",
      case 
        when is_investor_bool then null
      ...
      is_investor_bool as "isInvestorViewer",
    ...
    where business.status = 'active'
      -- Jika Investor: HANYA tampilkan yang opted_in = true
      -- Jika Lembaga: Tampilkan seluruh UMKM aktif tanpa filter opt-in
      and (
        not is_investor_bool 
