...
import { cancelLedgerTransaction, listTransactionChanges, updateLedgerTransaction } from "@/modules/ledger/ledger-repository";
...
/** Riwayat perubahan transaksi ini: siapa pun di usaha ini boleh membacanya. */
export async function GET(_request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    const user = await getAuthenticatedUser(); if (!user) throw new LedgerOperationError("UNAUTHENTICATED");
    const { id } = await context.params; const parsedId = transactionIdSchema.safeParse(id);
    if (!parsedId.success) return ledgerValidationErrorResponse(parsedId.error);
    return Response.json({ data: await listTransactionChanges(parsedId.data) }, { headers: { "Cache-Control": "private, no-store" } });
  } catch (error) { return ledgerErrorResponse(error); }
}
