...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  if (error || !dossier) return gagal("PROFILE_NOT_AVAILABLE", 404);
  const { data: grant } = await client.from("consent_grants").select("scopes,status,expires_at").eq("id", dossier.grant_id).maybeSingle();
  if (!grant || grant.status !== "active" || (grant.expires_at && new Date(grant.expires_at) <= new Date())) return gagal("ACCESS_INACTIVE", 403);
