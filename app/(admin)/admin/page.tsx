...
export default function AdminPage() {
  ...
  return (
    <div className="space-y-8 animate-fade-in-up">
      {/* Header */}
      <div>
        <h1 className="font-headline text-2xl md:text-3xl font-extrabold text-[#1b2a3a]">Ringkasan admin</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {stats.map((s) => (
          <div key={s.label} className="bg-white rounded-2xl p-6 border border-slate-200/60 shadow-sm hover:shadow-md transition-shadow">
            ...
            <p className="text-3xl font-bold text-[#1b2a3a] font-headline">{s.value}</p>
      ...
      <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            { href: "/admin/rules", Icon: Sliders, label: "Aturan sistem", desc: "Modulasi bobot komponen Readiness Score", color: "#8b5cf6", hoverBorder: "hover:border-[#8b5cf6]/40" },
            ...
            <Link key={item.href} href={item.href}>
              <div className={`bg-white rounded-2xl p-6 border border-slate-200/60 shadow-sm hover:shadow-md transition-all cursor-pointer group ${item.hoverBorder}`}>
                ...
                <p className="font-bold text-[#1b2a3a] text-base group-hover:text-[#0b5f86] transition-colors">{item.label}</p>
