...
import { ArrowDownLeft, ArrowUpRight, BarChart3, CalendarCheck, Camera, CheckCircle2, Download, FileText, LoaderCircle, Paperclip, Pencil, Plus, Receipt, RefreshCcw, TrendingDown, TrendingUp, X, XCircle } from "lucide-react";
...
export default function LaporanPage() {
  ...
  return <DashboardPage>
    <PageHeader title="Buku kas & laporan" description="Pahami uang masuk, biaya, dan selisih usaha dari catatan yang sudah Anda konfirmasi." icon={BarChart3} actions={<><a href={`/api/v1/ledger/export?startDate=${range.startDate}&endDate=${range.endDate}`} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-[#a9ebd0] bg-[#edfbf5] px-3 text-xs font-bold text-[#0b7a55]"><Download size={14} /> Unduh data</a><button onClick={() => setShowClosing(true)} disabled={Boolean(todayClosing)} className="inline-flex min-h-11 items-center gap-2 rounded-xl border border-[#addcf4] bg-[#eef8fd] px-3 text-xs font-bold text-[#0b5f86] disabled:cursor-not-allowed disabled:opacity-50"><CalendarCheck size={14} /> {todayClosing ? "Kas sudah ditutup" : "Tutup kas"}</button><button onClick={openCreate} className="inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#0b5f86] px-3 text-xs font-bold text-white"><Plus size={14} /> Catat transaksi</button></>} />
    ...
    <nav aria-label="Tampilan laporan" className="flex gap-1 rounded-xl border border-[#e3e9f0] bg-white p-1 shadow-[0_5px_18px_rgba(27,42,58,.04)]">
      {([{ id: "month", label: "Bulan Ini" }, { id: "bank", label: "Untuk Bank" }, { id: "cash", label: "Buku Kas" }] as const).map((item) =>
        <button key={item.id} onClick={() => setTab(item.id)} aria-current={tab === item.id ? "page" : undefined} className={`min-h-11 flex-1 rounded-lg px-3 text-xs font-bold transition-colors ${tab === item.id ? "bg-[#eef8fd] text-[#0b5f86] shadow-sm" : "text-[#6e859e] hover:bg-[#f3f6f9]"}`}>{item.label}</button>)}
    </nav>
    ...
    {tab === "cash" && <>
    <section className="rounded-2xl border border-[#e3e9f0] bg-white p-3 shadow-[0_8px_28px_rgba(27,42,58,.04)]"><div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between"><div className="flex gap-2 overflow-x-auto">{([{ id: "today", label: "Hari ini" }, { id: "week", label: "7 hari" }, { id: "month", label: "Bulan ini" }] as const).map((item) => <button key={item.id} onClick={() => choosePreset(item.id)} className={`whitespace-nowrap rounded-full px-3 py-1.5 text-xs font-bold ${preset === item.id ? "bg-[#0b5f86] text-white" : "bg-[#f3f6f9] text-[#4a6280]"}`}>{item.label}</button>)}</div><div className="flex items-center gap-2"><input type="date" value={range.startDate} max={range.endDate} onChange={(e) => { setPreset("custom"); setRange((r) => ({ ...r, startDate: e.target.value })); }} className="min-h-11 rounded-lg border border-[#d5dfe9] px-2 text-xs" /><span className="text-xs text-[#6e859e]">sampai</span><input type="date" value={range.endDate} min={range.startDate} max={jakartaDate()} onChange={(e) => { setPreset("custom"); setRange((r) => ({ ...r, endDate: e.target.value })); }} className="min-h-11 rounded-lg border border-[#d5dfe9] px-2 text-xs" /><button onClick={() => void loadReport()} aria-label="Muat ulang" className="rounded-lg p-2 text-[#6e859e] hover:bg-[#f3f6f9]"><RefreshCcw size={15} /></button></div></div></section>
    {loading || !report ? <div role="status" className="flex items-center justify-center gap-2 rounded-2xl bg-white p-12 text-sm text-[#6e859e]"><LoaderCircle className="animate-spin" size={18} /> Memuat buku kas...</div> : <>
      ...
      <section className="space-y-3"><div className="flex items-center justify-between"><div><h2 className="text-sm font-bold text-[#1b2a3a]">Riwayat transaksi</h2><p className="mt-1 text-[11px] leading-relaxed text-[#6e859e]">Transaksi dibatalkan tetap terlihat sebagai riwayat.</p></div><span className="shrink-0 text-[11px] font-bold text-[#6e859e]">{report.transactions.length} catatan</span></div>
        {report.transactions.length === 0 ? <div className="rounded-2xl border border-dashed border-[#c8d3de] bg-white p-10 text-center"><Receipt className="mx-auto text-[#9fb0c2]" size={30} /><p className="mt-2 text-sm font-bold text-[#1b2a3a]">Belum ada transaksi</p><button onClick={openCreate} className="mt-3 min-h-11 text-xs font-bold text-[#0b5f86]">Catat transaksi pertama</button></div> : <div className="space-y-2">{report.transactions.map((transaction) => { const locked = closedDates.has(transaction.transactionDate); return <article key={transaction.id} className={`rounded-2xl border border-[#e3e9f0] bg-white p-4 shadow-[0_8px_28px_rgba(27,42,58,.04)] ${transaction.status === "cancelled" ? "opacity-60" : ""}`}><div className="flex items-start justify-between gap-3"><div className="flex min-w-0 gap-3"><div className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-xl ${transaction.transactionType === "income" ? "bg-[#edfbf5] text-[#0b7a55]" : "bg-[#fdf1f3] text-[#b4304a]"}`}>{transaction.transactionType === "income" ? <TrendingUp size={16} /> : <TrendingDown size={16} />}</div><div className="min-w-0"><h3 className={`truncate text-xs font-bold text-[#1b2a3a] ${transaction.status === "cancelled" ? "line-through" : ""}`}>{transaction.description}</h3><p className="mt-1 text-[10px] text-[#6e859e]">{transaction.transactionDate} · {transaction.categoryLabel} · {paymentMethodLabels[transaction.paymentMethod ?? "unknown"]}</p><div className="mt-1.5 flex flex-wrap gap-1.5">{locked && <StatusBadge tone="info">Kas sudah ditutup</StatusBadge>}{transaction.changeCount > 1 && <StatusBadge tone="attention">Pernah diubah</StatusBadge>}{transaction.attachmentCount > 0 && <StatusBadge tone="info"><Paperclip size={9} className="mr-0.5 inline" />{transaction.attachmentCount > 1 ? `${transaction.attachmentCount} bukti` : "Ada bukti"}</StatusBadge>}{transaction.status === "cancelled" && <StatusBadge tone="neutral">Dibatalkan</StatusBadge>}</div></div></div><div className="shrink-0 text-right"><p className={`text-sm font-bold tabular-nums ${transaction.transactionType === "income" ? "text-[#0b7a55]" : "text-[#b4304a]"}`}>{transaction.transactionType === "income" ? "+" : "-"}{formatIdr(transaction.amountIdr)}</p>{transaction.status === "confirmed" && <div className="mt-1 flex justify-end gap-1"><button onClick={() => setEvidenceFor((value) => value === transaction.id ? null : transaction.id)} disabled={busy} aria-label="Tambah bukti" className="grid h-9 w-9 place-items-center rounded-lg text-[#0b5f86] hover:bg-[#eef8fd] disabled:opacity-30"><Camera size={14} /></button><button onClick={() => openEdit(transaction)} disabled={locked || busy} aria-label="Ubah transaksi" className="grid h-9 w-9 place-items-center rounded-lg text-[#0b5f86] hover:bg-[#eef8fd] disabled:opacity-30"><Pencil size={13} /></button><button onClick={() => void cancelTransaction(transaction)} disabled={busy} aria-label="Batalkan transaksi" className="grid h-9 w-9 place-items-center rounded-lg text-[#b4304a] hover:bg-[#fdf1f3] disabled:opacity-30"><XCircle size={14} /></button></div>}</div></div>{evidenceFor === transaction.id && <div className="mt-3"><EvidencePrompt targets={[{ targetType: "transaction", targetId: transaction.id }]} title="Tambah bukti" hint="Foto nota atau kuitansinya. Bukti yang menempel di catatan ini yang nanti membuatnya bisa dipercaya pihak lain." buttonLabel="Pilih foto" onAttached={() => { setEvidenceFor(null); void loadReport(); }} /></div>}</article>; })}</div>}
      </section>
    </>}
    </>}
    ...
    {showClosing && <ClosingDialog dayLabel={closingDayLabel(closingDate, new Date())} openingCash={openingCash} physicalCash={physicalCash} note={closingNote} busy={busy} setOpeningCash={setOpeningCash} setPhysicalCash={setPhysicalCash} setNote={setClosingNote} onClose={() => setShowClosing(false)} onSubmit={closeToday} />}
  </DashboardPage>;
}
...
function TransactionDialog({ form, setForm, editing, busy, allowedCategories, onType, onClose, onSubmit }: { form: FormState; setForm: React.Dispatch<React.SetStateAction<FormState>>; editing: LedgerTransactionView | null; busy: boolean; allowedCategories: typeof categoryOptions[number][]; onType: (type: "income" | "expense") => void; onClose: () => void; onSubmit: (event: React.FormEvent) => void }) {
  return <div className="fixed inset-0 z-[60] flex items-end justify-center bg-slate-950/50 md:items-center md:p-6" role="dialog" aria-modal="true"><div className="max-h-[88dvh] w-full overflow-y-auto overscroll-contain rounded-t-3xl bg-white p-5 pb-[calc(env(safe-area-inset-bottom)+20px)] md:max-h-[88vh] md:max-w-xl md:rounded-3xl md:p-6"><div className="flex justify-between"><div><p className="text-[11px] font-bold uppercase tracking-wide text-[#0b5f86]">{editing ? "Ubah transaksi" : "Catat transaksi"}</p><h2 className="mt-1 text-lg font-bold text-[#1b2a3a]">{editing ? "Perbaiki catatan" : "Uang masuk atau keluar"}</h2></div><button onClick={onClose} aria-label="Tutup"><X size={18} /></button></div><form onSubmit={onSubmit} className="mt-5 space-y-4"><div className="grid grid-cols-2 gap-2"><button type="button" onClick={() => onType("income")} className={`rounded-xl border p-3 text-xs font-bold ${form.transactionType === "income" ? "border-[#a9ebd0] bg-[#edfbf5] text-[#0b7a55]" : "border-[#e3e9f0]"}`}>Pemasukan</button><button type="button" onClick={() => onType("expense")} className={`rounded-xl border p-3 text-xs font-bold ${form.transactionType === "expense" ? "border-[#9fb0c2] bg-[#f3f6f9] text-[#34496a]" : "border-[#e3e9f0]"}`}>Pengeluaran</button></div><Field label="Keterangan"><input required value={form.description} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} placeholder="Contoh: Penjualan 10 porsi" className="ledger-input" /></Field><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><Field label="Nominal"><InlineMoneyInput value={form.amount === "" ? null : Number(form.amount)} onChange={(value) => setForm((f) => ({ ...f, amount: value === null ? "" : String(value) }))} ariaLabel="Nominal transaksi" /></Field><Field label="Tanggal"><input required type="date" max={jakartaDate()} value={form.date} onChange={(e) => setForm((f) => ({ ...f, date: e.target.value }))} className="ledger-input" /></Field></div><Field label="Kategori"><select value={form.categoryCode} onChange={(e) => setForm((f) => ({ ...f, categoryCode: e.target.value }))} className="ledger-input">{allowedCategories.map((item) => <option key={item.code} value={item.code}>{item.label}</option>)}</select></Field><div className="grid grid-cols-1 gap-3 sm:grid-cols-2"><Field label="Pembayaran"><select value={form.paymentMethod} onChange={(e) => setForm((f) => ({ ...f, paymentMethod: e.target.value }))} className="ledger-input">{Object.entries(paymentMethodLabels).filter(([key]) => key !== "unknown").map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></Field><Field label="Pelanggan / pemasok"><input value={form.counterparty} onChange={(e) => setForm((f) => ({ ...f, counterparty: e.target.value }))} className="ledger-input" /></Field></div>{editing && <Field label="Alasan perubahan"><textarea required minLength={3} value={form.reason} onChange={(e) => setForm((f) => ({ ...f, reason: e.target.value }))} placeholder="Contoh: nominal salah ketik" className="ledger-input min-h-20" /></Field>}<button disabled={busy} className="flex min-h-11 w-full items-center justify-center gap-2 rounded-xl bg-[#0b5f86] p-3 text-xs font-bold text-white disabled:opacity-50">{busy ? <LoaderCircle className="animate-spin" size={15} /> : <CheckCircle2 size={15} />}{busy ? "Menyimpan..." : "Simpan transaksi"}</button></form></div></div>;
}
function ClosingDialog({ dayLabel, openingCash, physicalCash, note, busy, setOpeningCash, setPhysicalCash, setNote, onClose, onSubmit }: { dayLabel: string; openingCash: string; physicalCash: string; note: string; busy: boolean; setOpeningCash: (value: string) => void; setPhysicalCash: (value: string) => void; setNote: (value: string) => void; onClose: () => void; onSubmit: (event: React.FormEvent) => void }) { return <div className="fixed inset-0 z-[60] flex items-end justify-center bg-slate-950/50 md:items-center md:p-6" role="dialog" aria-modal="true" aria-labelledby="closing-dialog-title"><div className="max-h-[88dvh] w-full overflow-y-auto overscroll-contain rounded-t-3xl bg-white p-5 pb-[calc(env(safe-area-inset-bottom)+20px)] md:max-h-[88vh] md:max-w-md md:rounded-3xl md:p-6"><div className="flex justify-between"><div><p className="text-[11px] font-bold uppercase tracking-wide text-[#0b5f86]">Tutup kas</p><h2 id="closing-dialog-title" className="mt-1 text-lg font-bold text-[#1b2a3a]">Selesaikan catatan {dayLabel}</h2></div><button type="button" onClick={onClose} aria-label="Tutup dialog" className="grid size-11 place-items-center rounded-lg text-[#6e859e] hover:bg-[#f3f6f9]"><X size={18} /></button></div><p className="mt-2 text-xs leading-relaxed text-[#6e859e]">Boleh dilewati jika Anda belum menghitung uang fisik. Setelah ditutup, transaksi {dayLabel} tidak dapat diedit tetapi masih dapat dibatalkan dengan alasan.</p><form onSubmit={onSubmit} className="mt-5 space-y-4"><Field label="Uang kas awal (opsional)"><InlineMoneyInput value={openingCash === "" ? null : Number(openingCash)} onChange={(value) => setOpeningCash(value === null ? "" : String(value))} ariaLabel="Uang kas awal" /></Field><Field label="Uang fisik saat ini (opsional)"><InlineMoneyInput value={physicalCash === "" ? null : Number(physicalCash)} onChange={(value) => setPhysicalCash(value === null ? "" : String(value))} ariaLabel="Uang fisik saat ini" /></Field><Field label="Catatan (opsional)"><textarea value={note} onChange={(e) => setNote(e.target.value)} className="ledger-input min-h-20" /></Field><button disabled={busy} className="min-h-11 w-full rounded-xl bg-[#0b5f86] p-3 text-xs font-bold text-white disabled:opacity-50">{busy ? "Menyimpan..." : `Tutup kas ${dayLabel}`}</button></form></div></div>; }
function Field({ label, children }: { label: string; children: React.ReactNode }) { return <label className="block text-xs font-bold text-[#1b2a3a]">{label}<div className="mt-1.5 [&_.ledger-input]:w-full [&_.ledger-input]:rounded-xl [&_.ledger-input]:border [&_.ledger-input]:border-[#d5dfe9] [&_.ledger-input]:px-3 [&_.ledger-input]:py-2.5 [&_.ledger-input]:text-sm [&_.ledger-input]:font-medium [&_.ledger-input]:outline-none focus-within:[&_.ledger-input]:border-[#0b5f86]">{children}</div></label>; }
function Distribution({ title, items, maximum }: { title: string; items: Array<{ code: string; label: string; amountIdr: number }>; maximum: number }) { return <DashboardPanel><PanelHeader title={title} /><div className="p-4 md:p-5">{items.length === 0 ? <p className="text-xs text-[#6e859e]">Belum ada data pada periode ini.</p> : <div className="space-y-3">{items.slice(0, 6).map((item) => <div key={item.code}><div className="flex justify-between gap-3 text-xs"><span className="min-w-0 truncate font-semibold text-[#4a6280]">{item.label}</span><span className="shrink-0 font-bold tabular-nums text-[#1b2a3a]">{formatIdr(item.amountIdr)}</span></div><div className="mt-1 h-2 overflow-hidden rounded-full bg-[#eef2f6]"><div className="h-full rounded-full bg-[#1590c7]" style={{ width: `${Math.max(4, Math.round(item.amountIdr / maximum * 100))}%` }} /></div></div>)}</div>}</div></DashboardPanel>; }
