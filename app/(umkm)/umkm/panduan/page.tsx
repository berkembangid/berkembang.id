      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {guides.map((guide) => (
          <DashboardPanel key={guide.href} className="group p-5 transition-[border-color,box-shadow] hover:border-[#addcf4] hover:shadow-[0_12px_34px_rgba(21,144,199,.08)]">
            ...
            <p className="mt-1 min-h-11 text-xs leading-relaxed text-[#6e859e]">{guide.description}</p>
            <Link href={guide.href} className="mt-5 inline-flex min-h-11 items-center gap-2 text-xs font-bold text-[#0b5f86]">{guide.action}<ArrowRight size={14} /></Link>
          </DashboardPanel>
        ))}
      </div>
